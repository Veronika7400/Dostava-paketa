...
public class Isporuka2 implements IsporukaPaketa {
    ...
    for (Vozilo vozilo : DostavaPaketa.dohvatiVozila()) {
      ...
      if (zadnja != null) {
        if (!jeIsporuceno(zadnja)) {
          poredajPaketeIspravno(zadnja, vozilo);
  ...
  private void poredajPaketeIspravno(Voznja zadnja, Vozilo vozilo) {
    List<String> poredani = new ArrayList<>();
    trenutniGps = Klijent.parametri.getProperty("gps").trim();
    int broj = zadnja.getPaketiUVozilu().size();
    while (poredani.size() != broj) {
      String iduciPaket = odaberiIduciPaket(zadnja, vozilo);
      poredani.add(iduciPaket);
      zadnja.removePaketUVozilu(iduciPaket);
    }
    zadnja.update(poredani);
  }

  private String odaberiIduciPaket(Voznja zadnja, Vozilo vozilo) {
    String id = null;
    ...
    for (String idPaketa : zadnja.getPaketiUVozilu()) {
      ...
      String gps = dohvatiGpsPaketa(paket);
      double udaljenost = izracunajUdaljenost(trenutniGps, gps);
